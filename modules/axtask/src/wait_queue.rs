...
use crate::{current_run_queue, run_queue, AxTaskRef, CurrentTask};
...
impl WaitQueue {
    ...
    {
        loop {
            let mut rq_locked = current_run_queue().scheduler_lock().lock();
            if condition() {
                break;
            }
    ...
    pub fn notify_one(&self, resched: bool) -> bool {
        if let Some(task) = self.queue.lock().pop_front() {
            run_queue::unblock_one_task(task, resched);
            true
        } else {
            false
        }
